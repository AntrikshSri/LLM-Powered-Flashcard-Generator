...
def index():
    ...
    difficulty = ''
    error = None
    loading = False # indicate generation is in progress
    ...
    if request.method == 'POST':
        ...
        if action == 'generate':
            if not text:
                error = "Please enter some text to generate flashcards."
            else:
                loading = True
                try:
...
def export_csv():
    ...
    if not text:
        return "No text provided for export.", 400

    try:
        flashcards = generate_flashcards(text, language, difficulty)
        if not flashcards:
            return " No flashcards could be generated for export. Try with more input.", 400
